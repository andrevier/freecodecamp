...
# Store the last available id for the new notes.
last_id = 0
...
class Notebook:
  ...
  def modify_memo(self, note_id, memo) -> bool:
    '''Find the note with the id and change its memo to the given 
    value.'''
  ...
  def modify_tags(self, note_id, tags) -> bool:
    '''Find the note with the id and change its tag with the given
    value in tags.'''
    note = self._find_note(note_id)
    if note:
      note.tags = tags
